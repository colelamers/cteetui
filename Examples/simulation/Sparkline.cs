...
namespace Tui.Core
{
    ...
    // The tallest bar in the visible window is highlighted in PeakColor.
    // =========================================================================
    public class Sparkline : Widget
    {
        ...
        // Unicode eighth-block characters from empty to full
        ...
        // ---------------------------------------------------------------------
        // Append
        // Add a new sample value and scroll the chart left if full.
        // ---------------------------------------------------------------------
        public void Append(double value)
        {
            samples_.Enqueue(value);

            // Keep only as many samples as the chart can display
            ...
            // Auto-scale: track the highest value seen
            if (AutoScale)
            {
                if (value > max_)
                {
        ...
        public override void Draw(ScreenBuffer buf, int ox, int oy)
        {
            ...
            // Draw each sample as a block character
            for (int i = 0; i < arr.Length && i < chart_w; i++)
            {
                ...
                if (block >= k_blocks_.Length)
                {
                    block = k_blocks_.Length - 1;
                }
